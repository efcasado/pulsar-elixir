defmodule Pulsar.Producer.Worker do
  ...
  # Matches the `:level` default in Pulsar.Producer.Options, and zstd's own.
  ...
  # The wire carries the codec alone: a codec's options only shape the frame its compressor
  # emits, so they are split apart here rather than carried through the state as one value.
  defp split_compression({:zstd, opts}), do: {:zstd, Keyword.fetch!(opts, :level)}
  defp split_compression(codec), do: {codec, @default_zstd_level}
  ...
  defp maybe_compress(%Binary.MessageMetadata{compression: :ZSTD}, <<>>, level) do
    {:ok, context} = :zstd.context(:compress, %{pledgedSrcSize: 0, compressionLevel: level})
